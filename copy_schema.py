#copy_schema Copyright (C) 2026 PauloPost37
...
# Debugged and highly expanded by Gemini 3
...
def run_migration_task(schemas, oracle_conf, pg_conf):
    ...
    try:
        ...
        with open("migration_report.txt", "a") as f:
            f.write(f"Estimated data migration Errors: {insert_errors}\n")
            f.write(f"Table Migration Errors: {table_errors}\n")
            f.write(f"Sequence Migration Errors: {sequence_errors}\n")
            f.write(f"View Migration Errors: {view_errors}\n")
            f.write(f"Constraint and Index Migration errors: {ci_errors}\n")
            f.write(f"Total migration time: {total_migration_time}, Data Migration time: {total_time_data} \n")
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        try:
            ...
            print("copy_schema  Copyright (C) 2026  PauloPost37")
